...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Flatten, Dense, Input, Reshape # type: ignore
...
X = tf.reshape(X, [(X.shape[0]*X.shape[1]), X.shape[2], 1]) # (batch_size, length, channels)
...
# Define CNN
model = Sequential([
    Input(shape=(min_haplotype_length, 1)),
    ...
    Dense(3, activation='softmax')  # 3 classes: low, medium, high migration rate
])
